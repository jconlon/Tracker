...
public class DBObjectBuilder
{
	...
	public DBObject buildDBObject(EObject eObject)
	{
		...
		dbObject.put(Constants.ECLASS_KEY, EcoreUtil.getURI(eClass).toString());
	...
	protected void buildExtrensicID(EObject eObject, DBObject dbObject)
	{
		...
		if (resource instanceof XMLResource)
		{
			...
			if (id != null)
				dbObject.put(Constants.EXTRINSIC_ID_KEY, id);
		}
	}
	...
	protected DBObject buildReferencedObject(EReference eReference, EObject targetObject)
	{
		...
		if (eProxyURI != null)
		{
			...
			dbObject.put(Constants.PROXY_KEY, uriHandler.deresolve(eProxyURI).toString());
			dbObject.put(Constants.ECLASS_KEY, EcoreUtil.getURI(targetObject.eClass()).toString());
			return dbObject;
		}
		else if (!eReference.isContainment() || (eReference.isResolveProxies() && internalEObject.eDirectResource() != null))
		{
			...
			BasicDBObject dbObject = new BasicDBObject(2);
			dbObject.put(Constants.PROXY_KEY, uriHandler.deresolve(EcoreUtil.getURI(targetObject)).toString());
			dbObject.put(Constants.ECLASS_KEY, EcoreUtil.getURI(targetObject.eClass()).toString());
			return dbObject;
		}
		else
