...
__iRouter monitor view of an integrated multi-sensor data logger__
...
While it is possible to use multiple generic components for business logic, complex business logic can instead can be encapsulated in a single custom business logic component. This use case depicts using a single business logic component for integrating, controlling and transforming complex information network flows between multiple sensors and a data logger.
