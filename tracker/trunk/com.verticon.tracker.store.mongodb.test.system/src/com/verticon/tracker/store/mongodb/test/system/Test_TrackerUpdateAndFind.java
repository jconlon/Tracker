...
public class Test_TrackerUpdateAndFind extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		if (!initializedCollections) {
			initializedCollections = removeDocsFromCollections(true);
			ensureIndexes();
		}
	...
	static boolean removeDocsFromCollections(boolean removeOCD)
	...
	@Test
	public void test_TrackerUpdate_RegisterPremises()
			throws IOException {
		logger.debug("RegisterwithSimpleDocument");
		...
		logger.debug(bundleMarker, "RegisterWithComplexDocument");
		Resource resource = getXMIResource(DOC_PREMISES, "");
		premises = (Premises) resource.getContents().get(0);
		register(premises);


		logger.debug(bundleMarker, "RegisterMultiplePremises");
		resource = getXMIResource(DOC_PREMISES, "");
		premises = (Premises) resource.getContents().get(0);
	...
	// premises = trackerFind.retrievePremises(point);
	//
	// assertThat("Premises should not be null", premises, is(notNullValue()));
	// assertThat("Premises should have uri", premises.getUri(),
