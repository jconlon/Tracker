...
final class MongoDB {
	...
	private final AtomicReference<Mongo> mongoRef;

	/**
	 * Creates a new {@link MongoDB} instance.
	 * 
	 * @param serverNames
	 *            the space separated list of Mongo servers, cannot be
	 *            <code>null</code> or empty;
	 * @param dbName
	 *            the name of the MongoDB to connect to, cannot be
	 *            <code>null</code> or empty;
	 * @param collectionName
	 *            the name of the collection to use, cannot be <code>null</code>
	 *            or empty.
	 */
	public MongoDB(String serverNames, String dbName, String collectionName) {
		if (serverNames == null || "".equals(serverNames.trim())) {
			throw new IllegalArgumentException(
					"ServerNames cannot be null or empty!");
		}
		...
		this.mongoRef = new AtomicReference<Mongo>();

		this.servers = parseServers(serverNames);
		this.dbName = dbName;
	...
	public String toString() {
		return "MongoDB [servers=" + servers + ", dbName=" + dbName
				+ ", mongoRef=" + mongoRef + "]";
	}
	 ...
	 * @return <code>true</code> if the connection was succesful,
	 *         <code>false</code> otherwise.
	 */
	public boolean connect(String userName, String password) {
		Mongo newMongo = new Mongo(servers);

		Mongo oldMongo;
		do {
			oldMongo = mongoRef.get();
		} while (!mongoRef.compareAndSet(oldMongo, newMongo));

		DB db = newMongo.getDB(dbName);
	 ...
	 *             in case no connection to Mongo exists.
	 */
	...
	public void disconnect() {
		...
		if (mongo != null) {
			try {
				...
				mongoRef.compareAndSet(mongo, null);
			}
		}
	}
}
