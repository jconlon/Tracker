...
public class ImagePreviewPane extends Dialog {
	...
	private void createImagePreviewPane() {
		...
		try {
			URL url = new URL("platform:/plugin/com.verticon.tracker.irouter.monitor.view/icons/save_edit.gif");
			editImage = ImageDescriptor.createFromURL(url).createImage();
			saveItem.setImage( editImage);
	...
	public void open (Image image, Point size) {
 		...
 		try {
			...
			shellImage = ImageDescriptor.createFromURL(url).createImage();
